...
let ListOffersUseCase = class ListOffersUseCase {
    constructor(offersRepository) {
        this.offersRepository = offersRepository;
    }
    execute(sellerUUID, offerID) {
        return __awaiter(this, void 0, void 0, function* () {
            //   console.log(`USECASE: SellerUUID: ${sellerUUID}`);
            //   console.log(`USECASE: offerID: ${offerID}`);
            if (sellerUUID) {
                // console.log(`Case SellerUUID`);
                return yield this.offersRepository.listOffersBySellerUUID(sellerUUID);
            }
            else if (offerID) {
                // console.log(`Case OfferID`);
                let offer = yield this.offersRepository.listOfferByOfferID(offerID);
                return [offer];
            }
            else {
                // console.log(`Case all`);
                return yield this.offersRepository.list();
            }
        });
    }
};
ListOffersUseCase = __decorate([
